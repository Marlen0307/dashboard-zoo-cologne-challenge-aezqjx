...
- `npm run dev` does not work and the app does not start
- the linter, the terminal and thev dev tools warn about some undefined variable, `ANlMALS`
- the HTTP request status code is 500, so we have a problem in the server.
- the error messages indicate that the problem is in the api route in `animals.get.ts` file
- **Came back here after solving the first error** There is also another error, which basically says that `alert` is being called, but it is actually not defined. I suspect that this code (which is in `app.vue`) is being ran on the server, because `alert` only works on the browser.
- after running the app, I see some issue on the console warning me that I should set a CORS policy. Probably not a big issue on development, but on production this is sumething that needs to be handled.
...
- This is a page where information about the animals in the zoo are shown. The infos are layed out with a simple html table (this is done in the `TheAnimalTable.vue` component). This table holds some important info such as the species, gender, age (which is the animal birthday in ISO 8601 format, not a very user freindly way) and weight.
- for the first error, the only thing which was needed was to reference the variable correctly. So I simply replaced `ANlMALS` with `ANIMALS` (which was the correct name of the defined variable)
- for the second error, like I suspected, the code was first being ran on the server, where the `alert` is undefined. Therefore, I could have just removed the call to `alert` (it does not look to be doing something important), but in case it was really needed I added a check in order to run the code only on the client side, where the alert is defined.
  \*\*
...
// Your solution
The documentation is on the README.md file in the root directory of the project.
